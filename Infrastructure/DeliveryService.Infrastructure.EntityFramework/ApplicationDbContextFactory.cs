...
public class ApplicationDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
{
    ...
    /// <summary>
    /// Ищет каталог <c>Presentation/DeliveryService.WebHost</c> с <c>appsettings.json</c>,
    /// поднимаясь от текущей рабочей директории (как при запуске <c>dotnet ef</c> из корня решения).
    /// Дополнительно: переменная <c>DELIVERY_SERVICE_WEBHOST_PATH</c> — абсолютный путь к папке WebHost.
    /// </summary>
    private static string ResolveWebHostDirectory()
    {
        var explicitPath = Environment.GetEnvironmentVariable("DELIVERY_SERVICE_WEBHOST_PATH");
        ...
        {
            var nested = Path.Combine(dir.FullName, "Presentation", "DeliveryService.WebHost", "appsettings.json");
            if (File.Exists(nested))
                return Path.GetDirectoryName(nested)!;

            if (string.Equals(dir.Name, "DeliveryService.WebHost", StringComparison.OrdinalIgnoreCase)
        ...
        throw new InvalidOperationException(
            ...
            "(где есть папка Presentation/DeliveryService.WebHost), либо задайте " +
            "DELIVERY_SERVICE_WEBHOST_PATH на каталог с appsettings.json, либо POSTGRES_CONNECTION_STRING.");
    }
}
